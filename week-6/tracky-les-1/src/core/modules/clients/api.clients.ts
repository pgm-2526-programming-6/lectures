...
export const createClient = async (body: CreateClientBody): Promise<Client | null> => {
  ...
  const { data } = await API.from("clients").insert(body).select().single().throwOnError();
  return data;
};
...
export const updateClient = async (body: UpdateClientBody): Promise<Client | null> => {
  ...
  const { data } = await API.from("clients").update(body).eq("id", body.id).select().single().throwOnError();
  return data;
};
